...
- `IsQuarantined`, `QuarantineReason`: 실행 제외 상태
- 화면 표시용 분류, 메시지, 오버레이 문구

`IsExecutable`은 동작 종류가 있고 격리되지 않은 이벤트만 참이 됩니다.
...
7. 준비 실패, 중단 또는 저장 실패가 있으면 성공으로 표시하지 않고 `Failed` 또는 검토 전용 상태로 남깁니다.
...
5. 드래그는 시작·끝 좌표를 로그에 남기지만 현재 실행기가 재생하지 못하므로 격리합니다.
...
- 격리 이벤트는 실행용 오버레이에서 제외하거나 별도 경고로 표시합니다.
...
1. 프로젝트가 정상 완료 상태이고 영상이 준비되었는지 확인합니다.
2. 긴급 중지 키, 녹화 범위 밖 좌표, 미완료 녹화 이벤트를 격리합니다.
3. 주 모니터의 현재 위치와 크기가 녹화 당시 범위와 같은지 검사합니다.
4. 실행 직전 최신 프로젝트를 저장합니다.
5. 시작 지연 중 대상 입력 창을 기억합니다.
6. `Stopwatch`를 시작하고 각 `Offset`까지 기다립니다.
7. 텍스트·키는 `SharpHook EventSimulator`, 마우스는 Windows 입력 API로 전송합니다.
8. 취소 토큰 또는 `Ctrl + Shift + F12`가 감지되면 다음 이벤트 전송을 중단합니다.
...
- 앱 자신의 창에서 발생한 입력 제외
- 주 모니터 녹화 범위 밖 이벤트 격리
- 드래그 이벤트 격리
- 미완료·실패 녹화의 실행 제한
- 녹화 당시 화면 범위와 현재 범위 비교
- 키보드 입력 대상 foreground 창 확인
- 실행 직전 저장 실패 시 실행 중단
- 녹화·매크로 실행 중 편집 UI 잠금
- 전역 `Ctrl + Shift + F12` 중지

이 안전 장치들은 위험을 줄이지만, 외부 프로그램이 수행한 작업을 복구하거나 사용자의 검토를 대신하지는 않습니다.
...
- 오디오는 녹음하지 않습니다.
- 개인정보 자동 탐지·마스킹·암호화 저장 기능은 없습니다.
