...
if(smoke){app.setPath('userData',path.join(app.getPath('temp'),'gonggong-ax-smoke-'+process.pid));}
if(!app.requestSingleInstanceLock())app.quit();
 ...
 const win=new BrowserWindow({show:!smoke,width:430,height:760,minWidth:380,minHeight:600,backgroundColor:'#edf2f7',title:'공공 AX · Tape Studio',autoHideMenuBar:true,titleBarStyle:'hidden',titleBarOverlay:{color:'#183653',symbolColor:'#e4edf6',height:38},roundedCorners:true,webPreferences:{nodeIntegration:false,contextIsolation:true,sandbox:true,preload:path.join(__dirname,'preload.cjs')}});
 ...
 win.webContents.once('did-finish-load',()=>{if(smoke)require('./package-smoke.cjs')({app,win,bridge,getState:()=>state});else{win.show();win.focus();}});
